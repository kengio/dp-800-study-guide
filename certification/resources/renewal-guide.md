...
1. **Note the blueprint date your original exam covered.** The official skills-measured page shows the current "Skills measured as of …" date at the top. Compare it to the version that was current when you sat the exam. (If you don't remember, your Microsoft Learn certificate PDF lists the exam date. Microsoft's docs-authoring repo is no longer publicly browsable, so cross-reference that date against this study guide's own [`CHANGELOG.md`](../../CHANGELOG.md) instead — entries are dated and track blueprint-relevant changes over time.)
